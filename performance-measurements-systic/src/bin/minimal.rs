...
mod app {
    ...
    fn task7(mut cx: task7::Context) {
        let time_task_7_started = unsafe {
            let timer_reg_ptr: *const usize = 0xE000E018 as *const usize;
            read_volatile(timer_reg_ptr)
        };
        ...
        let difference =  time_task_6_ended - time_task_7_started;
        
        defmt::info!("Context switch time lower prio task: {:?}", difference);


        task8::spawn().ok();

        let time_task_7_ended = unsafe {
            let timer_reg_ptr: *const usize = 0xE000E018 as *const usize;
            read_volatile(timer_reg_ptr)
        };
    ...
    fn task8(mut cx: task8::Context) {
                let time_task_8_started = unsafe {
                    let timer_reg_ptr: *const usize = 0xE000E018 as *const usize;
                    read_volatile(timer_reg_ptr)
                };
        ...
        let difference = time_task_7_ended - time_task_8_started;

        defmt::info!("Context switch time same prio task: {:?}", difference);
